...
<head>
    ...
    <title>Inscription Pèlerin - {{ config('app.name') }}</title>
...
<body class="min-h-screen flex items-center justify-center py-12 px-4">
    <div class="max-w-md w-full space-y-6 bg-white p-8 rounded-2xl shadow-2xl">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 font-poppins">Inscription Pèlerin</h2>
            <p class="mt-1 text-sm text-gray-600">Vous recevrez un lien d'activation par email (valide 24h).</p>
        </div>

        <form action="{{ route('register.pelerin.store') }}" method="POST" class="space-y-4">
            @csrf
