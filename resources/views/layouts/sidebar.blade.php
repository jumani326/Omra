<aside class="sidebar text-white" id="sidebar">
    <div class="p-4">
        ...
        <nav class="space-y-2">
            ...
            @php $clientPilgrim = \App\Models\Pilgrim::where('email', auth()->user()->email)->first(); @endphp
            @if($clientPilgrim)
            <a href="{{ route('pilgrims.show', $clientPilgrim) }}" class="flex items-center px-4 py-3 rounded-lg hover:bg-dark-green transition {{ request()->routeIs('pilgrims.show') ? 'bg-dark-green' : '' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                </svg>
                <span class="sidebar-text">Mon dossier</span>
            </a>
            @endif
            @else
            @can('view-pilgrims')
